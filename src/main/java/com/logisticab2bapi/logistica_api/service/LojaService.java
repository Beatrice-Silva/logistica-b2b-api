...
public class LojaService {
    ...
    public Loja atualizar(Long id, Loja nova, Usuario logado){
        ...
        if(logado.getPerfilRole() != Usuario.PerfilRole.ADMIN && !atual.getIdUsuario().equals(logado.getId())){
    ...
    public Loja arquivar(Long id, Usuario usuarioLogado){
        ...
        if(usuarioLogado.getPerfilRole() != Usuario.PerfilRole.ADMIN && !l.getIdUsuario().equals(usuarioLogado.getId())){
            throw new ResponseStatusException(HttpStatus.FORBIDDEN);
        }
        l.setAtivo(false);
        return lojaRepo.save(l);
